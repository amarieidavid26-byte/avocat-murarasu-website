...
def fix_image_attributes(content, filepath):
    """Fix malformed image attributes"""
    updated_count = 0
    
    # Find all local images with malformed attributes
    pattern = r'<img([^>]*?)src="/public/images/([^"]*?)"([^>]*?)>'
    ...
    def replace_img(match):
        ...
        all_attrs = before_src + after_src
        all_attrs = re.sub(r'\s+src="/public/images/[^"]*"', '', all_attrs)  # Remove src
        all_attrs = re.sub(r'\s+(loading|decoding|fetchpriority|width|height|onerror)=["\'][^"\']*["\']', '', all_attrs)  # Remove existing performance attrs
        all_attrs = re.sub(r'\s+', ' ', all_attrs).strip()  # Clean whitespace
        ...
        if all_attrs:
            all_attrs = all_attrs + ' ' + ' '.join(attrs)
        else:
...
def process_file(filepath):
    ...
    try:
        ...
        updated_content, img_count = fix_image_attributes(content, filepath)
...
for filepath in html_files:
    ...
    if img_count > 0 or filepath in HERO_IMAGES:
        print(f"✓ {filepath}: {img_count} images fixed, preload: {preload_status[filepath]}")

print(f"\nTotal images fixed: {total_updated}")
